...
We extend the analysis to noise regimes.  Under Gaussian input noise, the deep network's
...
in all three noise regimes (H5).  A complexity scaling experiment (Phase 6) gives
...
Both attacks locate $\epst \approx 0.05$ on the coarse grid $\{0.001,0.01,0.05\}$, within $1.25\times$ of the theoretical $2^{-4} = 0.0625$. Phase~6 refines this with a finer $\varepsilon$ grid and finds $\epst \approx 0.03$ for $k=4$ (Table~\ref{tab:scaling}).
...
\begin{enumerate}
  ...
  \item \textbf{H4}: Critical adversarial budget $\epst \approx 0.05$ is close to the $2^{-k}$ scale.
